...
export class LazyLoadingRepository implements LazyLoadingInterface {
    ...
    try {
      ...
      const children = await Promise.all(
        firstLevelChildren.map(async (item) => {
          const firstLevelChildrensChildren =
  ...
  async loadByKey(
    ...
    try {
      ...
      if (!node.length) {
        throw new HttpException(
          { key: I18NEnums.CLASSIFICATION_NOT_FOUND, args: { key } },
          HttpStatus.NOT_FOUND,
        );
      }
  ...
  async loadClassificationWithPath(
    path: string[],
    realm: string,
    language: string,
  ) {
    try {
      const rootWithChildren: any =
        await this.getClassificationRootAndChildrenByLanguageAndRealm(
          realm,
          language,
        );
  ...
  async getClassificationRootAndChildrenByLanguageAndRealm(
    realm: string,
    language: string,
  ) {
    try {
      ...
      const childrenByLanguage = firstNodes.filter((item) =>
        item['_fields'][1]['labels'][0].endsWith('_' + language),
      );
  ...
  async loadClassification(key: string, header) {
    try {
      ...
      if (!node) {
        throw new HttpException(
          { key: I18NEnums.CLASSIFICATION_NOT_FOUND, args: { key: key } },
          HttpStatus.NOT_FOUND,
        );
      }
      const temp2 =
      ...
      for (const item of children) {
        ...
        item.leaf =
          childrenOfItem.map((item) => item.get('children')).length <= 0;
      }
  ...
  async loadClassificationWithPathByIsActive(
    path: string[],
    realm: string,
    language: string,
    isActive: boolean,
    ...
    try {
      const rootWithChildren: any =
        await this.getClassificationRootAndChildrenByLanguageAndRealmAndIsActive(
          realm,
          language,
          isActive,
        );
      ...
      for (const item of path) {
        const loadedChildren = await this.loadClassificationByIsActive(
          item,
          null,
          isActive,
        );
